"""Unit tests for the shared strategy math (ported/adapted from PR#3).

These test the pure utilities in options_trader/signals/math.py.
Run: python -m pytest tests/test_signals_math.py -q  (or unittest)

The math is designed to be reusable for both vertical spreads (existing logic)
and single-leg MCP paths.
"""

import unittest

# Direct submodule import (avoids triggering pandas dep in package __init__ during test)
from options_trader.signals.math import (
    LiquidityRules,
    SingleLegTradePlan,
...
class TestBuildSingleLegPlan(unittest.TestCase):
    """Tests for the single-leg builder (pragmatic MCP path from PR#3)."""
    ...
    def test_qualifying_trade_builds_plan(self):
        plan = build_single_leg_plan(**self.BASE)
        self.assertIsNotNone(plan)
        self.assertIsInstance(plan, SingleLegTradePlan)
    ...
    def test_negative_ev_rejected(self):
        ...
        plan = build_single_leg_plan(**{**self.BASE, "strike": 115.0, "bid": 0.30,
                                        "ask": 0.32})
        self.assertIsNone(plan)

    def test_illiquid_rejected(self):
        plan = build_single_leg_plan(**{**self.BASE, "open_interest": 0, "volume": 0})
        self.assertIsNone(plan)

    def test_account_too_small_rejected(self):
        # $5 account can't risk $50 within 2% -> zero contracts -> None
        plan = build_single_leg_plan(**{**self.BASE, "account_value": 5})
        self.assertIsNone(plan)
